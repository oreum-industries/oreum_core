...
class TextCleaner():
    ...
    def __init__(self):
        ...
        self.rx_numbers = re.compile(r'(\b[0-9,.\-\\\/]+\b)')                   # number blocks, money amounts, years etc
        self.rx_punct = re.compile(r'[{}]+'.format(
                                            re.escape(string.punctuation)))     # regular punctuation   
        self.rx_neg_apostrophe = re.compile(r"""\b(ca|do|wo|is|are|was|does|
                    shall|should|would|could|must|ai)(n)(?:\')(t)\b""", re.I)   # find apsotrophes in negations
        ...
        self.rx_htmlcom = re.compile(re.escape('<!--') + "(.*?)" +              
                                     re.escape('-->'), re.DOTALL)                # HTML comments (usually embedded CSS)
        self.rx_email = re.compile(r"""\b[a-z0-9\'._%+-]+@[a-z0-9.-]+
                                    \.[a-z]{2,4}\b""", re.I)                    # email addresses
        self.rx_web = re.compile(r"""((http[s]?\:\/\/)?(www\.)+
                                    [a-z0-9\-%\/.]+)""", re.I)                  # web addresses
        self.postcode = re.compile(
            r"""(GIR ?0AA|[A-PR-UWYZ]([0-9]{1,2}|([A-HK-Y][0-9]([0-9ABEHMNPRV-Y])?)
                |[0-9][A-HJKPS-UW])?[0-9][ABD-HJLNP-UW-Z]{2})""",re.I)
        self.natins = re.compile(r'\s*[a-zA-Z]{2}(?:\s*\d\s*){6}[a-zA-Z]?\s*')  # UK national insurance ID number
        self.phoneno= re.compile(
            r"""\(?(?:(?:0(?:0|11)\)?[\s-]?\(?|\+)44\)?[\s-]?\(?(?:0\)?[\s-]?
                \(?)?|0)(?:\d{2}\)?[\s-]?\d{4}[\s-]?\d{4}|\d{3}\)?[\s-]?\d{3}
                [\s-]?\d{3,4}|\d{4}\)?[\s-]?(?:\d{5}|\d{3}[\s-]?\d{3})|\d{5}\)?
                [\s-]?\d{4,5}|8(?:00[\s-]?11[\s-]?11|45[\s-]?46[\s-]?4\d))
                (?:(?:[\s-]?(?:x|ext\.?\s?|\#)\d+)?)""")                        # phone number
        self.nhs = re.compile(r'\d{3}\s?\d{3}\s?\d{4}')                         # UK NHS ID number
        # self.rx_nonchar = re.compile(r'[\d{}]+'.format(
        #     re.escape(string.punctuation)))
        self.rx_num_m = re.compile(r'^(?P<mill>[0-9]+?(?:[.]+?[0-9]+?)*?)m$', re.I)     # (1.4)M
        self.rx_num_k = re.compile(r'^(?P<thou>[0-9]+?(?:[.]+?[0-9]+?)*?)k$', re.I)     # (400)k
        self.rx_num = re.compile(r'^(?P<whol>[0-9]+?)(?P<frac>[.]+?[0-9]+?)*?$', re.I)  # (81)(.23)
...
class StopWorder():
    ...
    def __init__(self):
        ...
        self.rx_nonchar = re.compile(r'[\d{}]+'.format(
                                            re.escape(string.punctuation)))     # punctuation or numbers
    ...
    def _get_names(self):
        ...
        with open(self.names_first, 'r') as f:
            fns = ['{}{}'.format(x[0].upper(), x.strip()[1:]) for x in f.readlines()][1:]

        with open(self.names_last, 'r') as f:
            lns = ['{}{}'.format(x[0].upper(), x.strip()[1:]) for x in f.readlines()][1:]
    ...
    def _get_cities1000(self):
        ...
        with open(self.global_cities,'r') as f:
            cities = ['{}{}'.format(x[0].upper(),x.strip()[1:]) for x in f.readlines()][1:]
        f.close()
    ...
    def _get_unix_words(self):
        ...
        with open(self.unix_words,'r') as f:
            wrds = ['{}'.format(x.strip().lower()) for x in f.readlines()\
    ...
    def _get_email_filler(self):
        ...
        abv_days = ['mon', 'tues', 'tue', 'weds', 'wed'
                    ,'thurs', 'thur', 'fri', 'sat', 'sun']

        months = ['january', 'february', 'march', 'april', 'may', 'june', 'july'
                ,'august', 'september', 'october', 'november', 'december']

        abv_months = ['jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug'
                    ,'sept', 'sep', 'oct', 'nov', 'dec']

        email_cruft = ['from', 'frm', 'to', 're', 'fwd', 'fw', 'forward'
                        ,'forwarded', 'cc', 'bcc', 'sent', 'subject', 'subj'
                        ,'reply-to', '<mime-attachment.gif>']

        email_cruft_full = email_cruft + ['{}:'.format(w) for w in email_cruft]

        html_cruft = ['font']

        contact_info = [ 'email', 'e-mail', 'e', 'mail', 'mobile', 'mob'
            ,'m', 'telephone', 'phone', 'tel', 't', 'fax', 'http', 'https', 'www'
            ,'website', 'web', 'url', 'address', 'add', 'switchboard'
            ,'mob', 'tel', 'web']

        sals = ['dear', 'hello', 'hi', 'hey', 'thanks for']

        vals = ['sincerely', 'faithfully', 'kind regards', 'kindest regards'
                ,'best regards', 'warm regards', 'regards', 'rgds', 'thank you'
                ,'many thanks', 'much thanks', 'thanks', 'cheers']

        yet_more = ['mr', 'mrs', 'ms', 'attached', 'ok', 'okay', 'ha'
                ,'wa', 'date', 'let', 'message']


        filler = (days + abv_days + months + abv_months + email_cruft_full +
                html_cruft + contact_info + sals + vals + yet_more)
    ...
    def create_bloomfilter(self, wordset):
        ...
        print('Filter has {:.1e} bits, capacity for {} items'.format(
    ...
    def tokenise_filter_lem(self, txt, bloom, drop_rem=True, lem=True):
        ...
        ion_to_e = set(['laceration', 'punctuation', 'penetration', 
                        'operation', 'irritation', 'aggravation', 
                        'dislocation', 'location', 'amputation'])
