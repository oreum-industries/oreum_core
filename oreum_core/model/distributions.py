...
"""Deprecated as-at 2023-06-30 until I port v3 into v5"""
...
#     Copy from pymc.bound pymc.distributions.dist_math.py
#     """
#     return pt.switch(alltrue_elemwise(conditions), vals, 0.0)
...
#         loginvcdf = pt.log(mu) + pt.log(1 - (beta * pt.log(-pt.log(value)) / mu))
...
#     Mean      :math:`{\begin{cases}\ m+s\Gamma \left(1-{\frac  {1}{\alpha }}\right)&{\text{for }}\alpha >1\\\ \infty &{\text{otherwise}}\end{cases}}` # noqa: W505
...
#         self.alpha = alpha = pt.as_tensor_variable(floatX(alpha))
#         self.s = s = pt.as_tensor_variable(floatX(s))
#         self.mode = s * pt.power(alpha / (1.0 + alpha), 1.0 / alpha)
...
#         alpha = self.alpha
#         s = self.s
#         fn = pt.exp(-pt.power(value / s, -alpha))
...
#         )
#         fn = s * pt.power(-pt.log(value), -1.0 / alpha)
...
#         alpha = self.alpha
#         s = self.s
#         fn = pt.log(s) - (1.0 / alpha) * pt.log(-pt.log(value))
...
#         self.psi = psi = pt.as_tensor_variable(floatX(psi))
#         self.alpha = alpha = pt.as_tensor_variable(floatX(alpha))
#         self.s = s = pt.as_tensor_variable(floatX(s))
...
#         psi = self.psi
#         logp_ = pt.switch(
#             pt.neq(value, 0),  # or use pt.gt(value, 0), dunno which faster
#             pt.log(psi) + self.invweibull.logp(value),
#             pt.log1p(-psi),
#         )
...
#         loginvcdf = (1 / a) * pt.log(1 - (1 - value) ** (1 / b))
...
#         mu = self.mu
#         sigma = self.sigma
#         z = (pt.log(value) - mu) / sigma
#         fn = 0.5 * pt.erfc(-z / pt.sqrt(2.0))
...
#         mu = self.mu
#         sigma = self.sigma
#         # value = pt.clip(value, CLIP_U_AWAY_FROM_ZERO_ONE_FOR_INVCDFS, 1-CLIP_U_AWAY_FROM_ZERO_ONE_FOR_INVCDFS)
...
#         fn = pt.exp(mu - sigma * pt.sqrt(2) * pt.erfcinv(2 * value))
...
#         self.dist_natural = {
#             'pdf': r"""f(x \mid \mu, \sigma) = \frac{1}{x \sigma \sqrt{2 \pi}} \exp \left( -{ \frac{(\log{x} - \mu)^{2}}{2 \sigma^{2}}} \right) # noqa: W505
#                                              = \frac{1}{x \sigma \sqrt{2 \pi}} \exp - \left(\frac{\log{x}-\mu}{\sigma \sqrt{2}} \right)^{2}""", # noqa: W505
...
#         }
#         self.dist_log = {
#             'logpdf': r'\log f(x \mid \mu, \sigma) = - \frac{1}{2 \sigma^2} \log{(x-\mu)^{2}} + \frac{1}{2} \log{\frac{1}{2 \pi \sigma^{2}}} -\log{x}', # noqa: W505
#             'logcdf': r'\log F(x \mid \mu, \sigma) = \log \left[\frac{1}{2} \text{erfc} \left( \frac{\log{(x)} -\mu}{\sigma \sqrt{2}} \right) \right]', # noqa: W505
...
#         self.mu = mu = pt.as_tensor_variable(floatX(mu))
#         self.tau = tau = pt.as_tensor_variable(tau)
#         self.sigma = self.sd = sigma = pt.as_tensor_variable(sigma)
...
#         logp_val = pt.switch(
#             pt.gt(value, 0), pt.log(psi) + self.lognormal.logp(value), pt.log1p(-psi)
#         )
...
#             pt.log(psi) + self.lognormal.logcdf(value), pt.log1p(-psi)
#         )
...
#             1 - CLIP_U_AWAY_FROM_ZERO_ONE_FOR_INVCDFS,
#         )
#         fn = mu - sigma * pt.sqrt(2.0) * pt.erfcinv(2.0 * value)
...
#         mu = self.mu
#         sigma = self.sigma
#         fn = np.log(mu - sigma * pt.sqrt(2.0) * pt.erfcinv(2.0 * value))
...
#             'logcdf': r'\log F(x \mid \mu, \sigma) = \log{(\frac{1}{2})} + \log{\left[ \text{erfc} \left(- \frac{x - \mu}{\sigma \sqrt{2}} \right) \right]}', # noqa: W505
